/*
 * Project name: Programmable relay switch
 * Description:  Digital timer for a relay switch with cyclic feature
     Author: Rajendra Bhatt, 2016.
     MCU:             PIC16F628A
     Oscillator:      XT, 4.0 MHz
 * Version 4: This version also includes cyclic feature where user can choose cyclic
 option to run the timer in a continuous loop.
*/
...
// Define Messages
 ...
 unsigned short Mode_SELECT = 0 ; // 0:ON, 1:OFF, 2 : Cycle
 ...
 void Disp_Time(){
   ...
   for(i=0; i<5; i++){
      if(Mode_SELECT == 0){  // Mode_SELECT=0 is OFF time SELECT_sw
       Lcd_Out(2,1, MSG2);
       Disp_Char(HHMM_Pos+i,OFF_Time[i]);
      }
      if(Mode_SELECT == 1){  // Mode_SELECT=0 is ON time SELECT_sw
       Lcd_Out(2,1, MSG3);
       Disp_Char(HHMM_Pos+i,ON_Time[i]);
      }
      if(Mode_SELECT == 2){  // Mode_SELECT=0 is ON time SELECT_sw
       Lcd_Out(2,1, MSG4);
...
void Store_tData(){
  ...
  EEPROM_Write(8,1);     // Address 8 stores EEPROM Write flag
...
void main() {
  ...
  Lcd_Cmd(_LCD_CLEAR);
  Disp_First_Row();
  // Check if there are pre-stored time data in EEPROM
  if(EEPROM_Read(8) == 1){
   Read_tData();
  }
  ...
  do {
   // Do the following if MENU switch is pressed and timer is not running
   if(!MENU_sw && !Timer_On){
     Delay_X();
     if(!Get_Input){
     ...
     if(Mode_SELECT==2){   // Set cyclic option
      Repeat_Cycle ++;
   ...
   // Do the following when Select switch is pressed and timer is not running
